...
router.get('/LaLiga1', async (req, res) => {
    try {
...
router.get('/LaLiga', async (req, res) => {
    try {
        ...
        const enrichedData = result.rows.map(row => {
            return {
                ...
                "date_of_establishment": {
                    "@type": "Date",
                    "value": row.date_of_establishment 
                },
                "main_sponsor": row.main_sponsor,
                "trophies_won": row.trophies_won,
                "current_captains": row.current_captains.map(captain => ({
                    "@type": "Person",
        ...
        res.json({ status: "OK", message: "Fetched all data", response: enrichedData });
    } catch (err) {
        res.status(500).json({ status: "Error", message: "Error in database", error: err.message});
    }
});
